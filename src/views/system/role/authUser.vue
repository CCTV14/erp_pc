...
<script>
import { findRoleUsered, authUserCancel } from "@/api/system/role";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      findRoleUsered(this.queryParams).then((response) => {
    ...
    handleSelectionChange(selection) {
      this.userIds = selection.map((item) => {
        return {
          sysRoleId: this.queryParams.roleId,
          userId: item.userId,
        };
      });
    ...
    cancelAuthUser(row) {
      ...
      this.$modal
        .confirm('确认要取消该用户"' + row.user.name + '"角色吗？')
        .then(function () {
