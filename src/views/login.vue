<template>
  <div class="login">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form">
      <h3 class="title">ERP管理系统</h3>
      <el-form-item prop="phoneNumber">
        <el-input v-model="loginForm.phoneNumber" type="text" auto-complete="off" placeholder="账号">
      ...
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          ...
          this.$store
            ...
            .then((resetPassword) => {
              this.$router.push({ path: resetPassword ? '/resetPassword' : (this.redirect || "/") }).catch(() => { });
            })
            .catch(() => {
              this.loading = false;
              if (this.captchaEnabled) {
