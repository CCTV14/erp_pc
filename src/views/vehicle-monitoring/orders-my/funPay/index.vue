    ...
    >
      ...
      <el-form-item class="flex-end">
        <el-button
          ...
          size="mini"
          @click="handleQuery"
          v-hasPermi="['vehicle:funpayordermy:query']"
          >搜索</el-button
        >
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['vehicle:funpayordermy:submit']"
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-delete"
            v-hasPermi="['vehicle:funpayordermy:delete']"
            v-if="
...
import {
  myFunPayOrderFindPageForManage,
  myFunpayOrderSubmitById,
  deleteFunPayOrder,
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      myFunPayOrderFindPageForManage(this.params).then((response) => {
    ...
    /** 提交 */
    handleSubmitById(id) {
      myFunpayOrderSubmitById(id).then((res) => {
        // 可能要做弹框显示操作...
        if (res.success) {
