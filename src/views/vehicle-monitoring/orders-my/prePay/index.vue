    ...
    >
      ...
      <el-form-item class="flex-end">
        <el-button
          ...
          size="mini"
          @click="handleQuery"
          v-hasPermi="['vehicle:prepayordermy:query']"
          >搜索</el-button
        >
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['vehicle:prepayordermy:submit']"
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-delete"
            v-hasPermi="['vehicle:prepayordermy:delete']"
            v-if="
...
import {
  myPrePayOrderFindPageForManage,
  myPrePayOrderSubmitById,
  deletePrePayOrder,
...
export default {
  ...
  data() {
    return {
      ...
      params: {
        quickSearchInfoList: [
          {
            columns: ["orderNo", "supplier.name", "remark", "creator.name"],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      myPrePayOrderFindPageForManage(this.params).then((response) => {
    ...
    /** 提交 */
    handleSubmitById(id) {
      myPrePayOrderSubmitById(id).then((res) => {
        // 可能要做弹框显示操作...
        if (res.success) {
