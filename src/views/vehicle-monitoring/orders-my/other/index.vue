    ...
    >
      ...
      <el-form-item class="flex-end">
        <el-button
          ...
          size="mini"
          @click="handleQuery"
          v-hasPermi="['vehicle:otherordermy:query']"
          >搜索</el-button
        >
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['vehicle:otherordermy:submit']"
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-delete"
            v-hasPermi="['vehicle:otherordermy:delete']"
            v-if="
...
import {
  myOtherOrderFindPageForManage,
  myOtherOrderSubmitById,
  deleteOtherOrder,
...
export default {
  ...
  data() {
    return {
      ...
      params: {
        quickSearchInfoList: [
          {
            columns: ["orderNo", "incomeType", "remark", "creator.name"],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      myOtherOrderFindPageForManage(this.params).then((response) => {
    ...
    /** 提交 */
    handleSubmitById(id) {
      myOtherOrderSubmitById(id).then((res) => {
        // 可能要做弹框显示操作...
        if (res.success) {
