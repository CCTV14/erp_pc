<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="入库类型" prop="commodityInputOrderTypeEnumList">
        <el-select
          v-model="params.commodityInputOrderTypeEnumList"
          collapse-tags
          multiple
          placeholder="请选择出库类型"
          clearable
        ...
        >
          <!-- <el-option label="不限" value=""></el-option> -->
          <el-option
            v-for="(item, index) in selectCommodityInputOrderTypeList"
            :key="index"
            :label="item.Desc"
            :value="item.Name"
          ></el-option>
    ...
    >
      ...
      <el-table-column
        label="关联销售退货单"
        prop="relatedSaleReturnOrderHead.orderNo"
        align="center"
        width="160"
      />
      <el-table-column
        label="关联采购单"
        align="center"
        prop="relatedPurchaseOrderHead.orderNo"
        width="160"
      />
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['vehicle:indelivermy:startdelivery']"
            v-if="canStartDelivery(scope.row)"
            @click="handleStartDelivery(scope.row.id)"
            >开始配送入库单</el-button
          >
          <el-button
            size="mini"
            ...
            v-hasPermi="['vehicle:indelivermy:completedelivery']"
            v-if="canCompleteDelivery(scope.row)"
            @click="handleSuccessDelivery(scope.row.id)"
            >完成配送入库单</el-button
          >
        </template>
      </el-table-column>
...
import {
  getCommodityInputOrderTypeEnumList,
  getCommodityDeliverStatusEnumList,
  getOrderApprovalStatusEnumList,
} from "@/api/vehicle-monitoring/order.js";

import { getMyInDeliverList } from "@/api/vehicle-monitoring/order-my.js";
...
export default {
  ...
  data() {
    return {
      ...
      // 入库类型
      selectCommodityInputOrderTypeList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层 新增/编辑框
      open: false,
      ...
      params: {
        ...
        quickSearchInfoList: [
          {
            columns: [
              ...
              "relatedSaleReturnOrderHead.orderNo",
              "relatedPurchaseOrderHead.orderNo",
              "remark",
              "creator.name",
            ],
        ...
        commodityInputOrderTypeEnumList: null,
  ...
  created() {
    ...
    this.flushCommodityInputOrderType();
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getMyInDeliverList(this.params).then((response) => {
    ...
    // 获取入库进度
    async flushCommodityInputOrderType() {
      let res = await getCommodityInputOrderTypeEnumList();
      if (res && res.success) {
        this.selectCommodityInputOrderTypeList = [
          {
            Desc: "不限",
            Name: null,
          },
    ...
    // 判断开始配送
    canStartDelivery(head) {
      // CommodityInputOutputOrder_Delivery
      return (
        head.orderApprovalStatusEnum.Name === "Approval" &&
        head.commodityDeliverStatusEnum.Name === "NotStart" &&
        head.commodityInputOrderTypeEnum.Name === "SaleReturnOrder" &&
        head.needDelivery &&
        head.deliveryUserList.filter((item) => item.userId === this.userInfo.id)
          .length > 0
      );
    },
    // 判断完成配送
    canCompleteDelivery(head) {
      // CommodityInputOutputOrder_Delivery
      return (
        head.orderApprovalStatusEnum.Name === "Approval" &&
        head.commodityDeliverStatusEnum.Name === "Delivering" &&
        head.commodityInputOrderTypeEnum.Name === "SaleReturnOrder" &&
        head.needDelivery &&
        head.deliveryUserList.filter((item) => item.userId === this.userInfo.id)
          .length > 0
