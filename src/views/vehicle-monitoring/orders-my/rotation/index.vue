<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item class="flex-end">
        <el-button
          ...
          size="mini"
          @click="handleQuery"
          v-hasPermi="['vehicle:rotationordermy:query']"
          >搜索</el-button
        >
    ...
    >
      ...
      <el-table-column label="转出账户" align="center" width="150">
        <template slot-scope="{ row }">
          {{
            "[" +
            row.outFundAccount.fundAccountTypeEnum.Desc +
            "] " +
            row.outFundAccount.name
          }}
        </template>
      </el-table-column>
      <el-table-column label="转入账户" align="center" width="150">
        <template slot-scope="{ row }">
          {{
            "[" +
            row.inFundAccount.fundAccountTypeEnum.Desc +
            "] " +
            row.inFundAccount.name
          }}
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['vehicle:rotationordermy:submit']"
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-delete"
            v-hasPermi="['vehicle:rotationordermy:delete']"
            v-if="
...
import {
  myRotationOrderFindPageForManage,
  myRotationOrderSubmitById,
  deleteRotationrder,
...
export default {
        ...
        quickSearchInfoList: [
          {
            columns: [
              "orderNo",
              "outFundAccount.name",
              "outFundAccount.account",
              "inFundAccount.name",
              "inFundAccount.account",
              "remark",
              "creator.name",
            ],
          ...
          {
            columns: ["outFundAccount.name", "outFundAccount.account"],
            quickSearchValue: null,
          },
          {
            columns: ["inFundAccount.name", "inFundAccount.account"],
            quickSearchValue: null,
          },
          {
            columns: ["remark"],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      myRotationOrderFindPageForManage(this.params).then((response) => {
    ...
    /** 提交 */
    handleSubmitById(id) {
      myRotationOrderSubmitById(id).then((res) => {
        // 可能要做弹框显示操作...
        if (res.success) {
