...
import {
  ...
  commodityLossOrderFindPageForManage,
} from "@/api/vehicle-monitoring/order.js";

import { commodityLossOrderExport } from "@/api/vehicle-monitoring/order";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      commodityLossOrderFindPageForManage(this.params).then((response) => {
    ...
    handleExport() {
      ...
      commodityLossOrderExport(this.params).then((res) => {
        if (res.success) {
          this.$modal.closeLoading();
          this.$modal.msgSuccess("导出成功");
          this.$thinking.downloadFileByByte(res.data,"报损单导出数据.xlsx");
        }
      });
    },
    // 取消按钮
