...
import {
  ...
  purchaseReturnOrderFindPageForManage,
} from "@/api/vehicle-monitoring/order.js";

import { purchaseReturnOrderExport } from "@/api/vehicle-monitoring/order";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      purchaseReturnOrderFindPageForManage(this.params).then((response) => {
    ...
    handleExport() {
     this.$modal.loading('正在导出...');
      purchaseReturnOrderExport(this.params).then((res) => {
        if (res.success) {
          this.$modal.closeLoading();
          this.$modal.msgSuccess("导出成功");ƒ
          this.$thinking.downloadFileByByte(res.data,"采购退货单导出数据.xlsx");
        }
      });
    },
    // 取消按钮
