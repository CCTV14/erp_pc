...
import {
  ...
  otherIncomeOrderFindPageForManage,
} from "@/api/vehicle-monitoring/order.js";

import { otherIncomeOrderExport } from "@/api/vehicle-monitoring/order";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      otherIncomeOrderFindPageForManage(this.params).then((response) => {
    ...
    handleExport() {
     this.$modal.loading('正在导出...');
      otherIncomeOrderExport(this.params).then((res) => {
        if (res.success) {
          this.$modal.closeLoading();
          this.$modal.msgSuccess("导出成功");ƒ
          this.$thinking.downloadFileByByte(res.data,"其他收入单导出数据.xlsx");
        }
      });
    },
    // 取消按钮
