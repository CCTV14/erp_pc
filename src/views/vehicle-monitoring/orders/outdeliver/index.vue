      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['vehicle:outdeliver:dis']"
            v-if="canDistributeDelivery(scope.row)"
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-circle-check"
            v-hasPermi="['vehicle:outdeliver:confirm']"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getOutDeliverList(this.params).then((response) => {
    ...
    canDistributeDelivery(head) {
      return (
        head.orderApprovalStatusEnum.Name === "Approval" &&
        head.commodityDeliverStatusEnum.Name === "Unassigned" &&
        this.isSaleOrderOutput(head) &&
        head.needDelivery
      );
    },
