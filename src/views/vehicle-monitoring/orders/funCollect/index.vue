<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <span v-if="row.fundCollectOrderTypeEnum.Name != 'SaleOrder'">
...
import {
  ...
  fundCollectOrderFindPageForManage,
} from "@/api/vehicle-monitoring/order.js";

import { fundCollectOrderExport } from "@/api/vehicle-monitoring/order";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      fundCollectOrderFindPageForManage(this.params).then((response) => {
    ...
    handleExport() {
     this.$modal.loading('正在导出...');
      fundCollectOrderExport(this.params).then((res) => {
        if (res.success) {
          this.$modal.closeLoading();
          this.$modal.msgSuccess("导出成功");ƒ
          this.$thinking.downloadFileByByte(res.data, "收款单导出数据.xlsx");
        }
      });
    },
    // 取消按钮
