<template>
  <div class="app-container home">
    <!-- 详情描述 -->
    <div class="head mb15">
      <el-descriptions title="费用支出单详情" :column="4">
            ...
            <el-descriptions-item label="资金账户">
              <div style="display: flex; flex-wrap: wrap">
                ...
                >
                  ...
                  <div class="mb5">
                    收入金额：￥{{ item.itemAmount | priceFixedTwo }}
                  </div>
                  <div>收款方名称：{{ item.remark }}</div>
                </div>
              </div>
            </el-descriptions-item>
      ...
      <el-col :span="12">
        <el-card class="box-card">
          ...
          <ImagePreview
            height="80px"
            width="80px"
            v-for="(item, index) in detailForm.expenseVoucherImageNameList"
            :key="index"
            order="expense"
            :src="item"
          />
...
<script>
import { getExpenseOrderFindById } from "@/api/vehicle-monitoring/order";
export default {
  data() {
    return {
      mode: "view",
  ...
  methods: {
    getDetail() {
      getExpenseOrderFindById({ id: this.$route.query.orderId }).then((res) => {
