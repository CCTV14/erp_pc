...
import {
  ...
  prePayOrderFindPageForManage,
} from "@/api/vehicle-monitoring/order.js";

import { prePayOrderExport } from "@/api/vehicle-monitoring/order";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      prePayOrderFindPageForManage(this.params).then((response) => {
    ...
    handleExport() {
      ...
      prePayOrderExport(this.params).then((res) => {
        if (res.success) {
          this.$modal.closeLoading();
          this.$modal.msgSuccess("导出成功");
          this.$thinking.downloadFileByByte(res.data,"预付单导出数据.xlsx");
        }
      });
    },
    // 取消按钮
