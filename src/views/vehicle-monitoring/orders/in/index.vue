...
import {
  ...
  commodityInputOrderFindPageForManage,
} from "@/api/vehicle-monitoring/order.js";

import { commodityInputOrderExport } from "@/api/vehicle-monitoring/order";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      commodityInputOrderFindPageForManage(this.params).then((response) => {
    ...
    handleExport() {
      ...
      commodityInputOrderExport(this.params).then((res) => {
        if (res.success) {
          this.$modal.closeLoading();
          this.$modal.msgSuccess("导出成功");
          ƒ;
          this.$thinking.downloadFileByByte(res.data, "入库单导出数据.xlsx");
        }
      });
    },
    // 取消按钮
