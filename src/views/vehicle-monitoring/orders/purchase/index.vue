...
import {
  ...
  purchaseOrderFindPageForManage,
} from "@/api/vehicle-monitoring/order.js";

import { purchaseOrderExport } from "@/api/vehicle-monitoring/order";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      purchaseOrderFindPageForManage(this.params).then((response) => {
    ...
    handleExport() {
      ...
      purchaseOrderExport(this.params).then((res) => {
        if (res.success) {
          this.$modal.closeLoading();
          this.$modal.msgSuccess("导出成功");
          ƒ;
          this.$thinking.downloadFileByByte(res.data, "采购单导出数据.xlsx");
        }
      });
    },
    // 取消按钮
