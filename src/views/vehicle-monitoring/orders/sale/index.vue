      ...
      >
        ...
        <el-row class="wd100 mb8">
          <el-col :span="12" class="flex-start">
            ...
            size="mini"
            @click="handleAdd"
            v-hasPermi="['vehicle:saleorder:add']"
            >新增</el-button
          > -->
            <el-button
              type="warning"
              ...
              size="mini"
              v-hasPermi="['vehicle:saleorder:export']"
              @click="handleExport"
              >导出</el-button
            >
          </el-col>
          <el-col :span="12" class="flex-end">
            <el-button
              ...
              icon="el-icon-search"
              size="mini"
              v-hasPermi="['vehicle:saleorder:query']"
              @click="handleQuery"
              >搜索</el-button
            >
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              v-hasPermi="['vehicle:saleorder:delete']"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
import { saleOrderExport } from "@/api/vehicle-monitoring/order";
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      ...
      saleOrderExport(this.params).then((res) => {
        if (res.success) {
          this.$modal.closeLoading();
          this.$modal.msgSuccess("导出成功");
          this.$thinking.downloadFileByByte(res.data, "销售单导出数据.xlsx");
        }
      });
    },
    // 取消按钮
